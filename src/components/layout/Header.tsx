    ...
    <header className="sticky top-0 z-40 bg-paper/80 backdrop-blur-md border-b border-rule">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="h-16 flex items-center justify-between gap-3">
          <Link href="/" className="group flex items-baseline gap-1 shrink-0">
          ...
          <nav className="flex items-center gap-3 sm:gap-7">
            {[
              ...
              <Link
                key={href}
                href={href}
                className="text-xs sm:text-sm font-medium text-ink-soft hover:text-gold transition-colors relative after:absolute after:left-0 after:-bottom-1.5 after:h-px after:w-0 after:bg-gold hover:after:w-full after:transition-all"
              >
                {label}
              </Link>
            ))}
